...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        """
        prediction, v_l = instances_to_coco_json_video(inputs, outputs)
    ...
    def _eval_predictions(self, predictions):
        ...
        if self._output_dir:
            ...
            with PathManager.open(file_path, "w") as f:
                ...
                f.flush()
            print('All frame num: ', self._frame_num)

        self._logger.info("Annotations are not available for evaluation.")
        return

def maskToAnn(mask):
    # mask: v_l, H, W -> length of video, original video height, width
    segmentations = []
    l = len(mask)
    for i in range(l):
        if mask[i].sum() == 0:
            ...
            rle = mask_utils.encode(np.asfortranarray(mask_data))
...
def instances_to_coco_json_video(inputs, outputs):
    ...
    scores = outputs["pred_scores"].data.cpu().numpy()
    labels = outputs["pred_labels"].data.cpu().numpy()
    masks = outputs["pred_masks"].data.cpu().numpy()
    ...
    for k in range(K):
        ...
        prediction['category_id'] = int(labels[k])
        prediction['score'] = float(scores[k])
        prediction['segmentations'] = maskToAnn(masks[k])
        predictions.append(prediction)

    return predictions, video_length
