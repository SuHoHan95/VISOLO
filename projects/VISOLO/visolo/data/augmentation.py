...
class ResizeShortestEdge(T.Augmentation):
    ...
    def get_transform(self, image):
        ...
        neww = int(neww + 0.5)
        newh = int(newh + 0.5)
        if newh < self.crop_h + 1:
            newh = self.crop_h + 1
        if neww < self.crop_w + 1:
            neww = self.crop_w + 1
...
def build_augmentation(cfg, is_train):
    ...
    if is_train:
        ...
        # Resize
        crop_h, crop_w = cfg.INPUT.CROP.SIZE
        ...
        if cfg.INPUT.RANDOM_ROTATION:
            aug_list.append(T.RandomRotation([-15., 15.]))
        ...
        # Resize
        new_size = cfg.INPUT.CROP.SIZE
        # min_size = cfg.INPUT.MIN_SIZE_TEST
        # max_size = cfg.INPUT.MAX_SIZE_TEST
        # sample_style = "choice"
        aug_list.append(T.Resize(new_size))
